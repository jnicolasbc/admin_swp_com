...
class AgendasController extends \BaseController {
    ...
    public function index()
    {
        ...
        $agendas = Agenda::where('doctor_id', $d->id)->get();

      return View::make('clinic.doctor.Agendas.index',['agendas'=>$agendas]);
  ...
  public function update($id)
  {
    ...
    public function getAgendaPatients($id)
    {
        if(Payment::VeryPayment()==false)
           return View::make('clinic.payment.renews-payment');
            ...
            $patients = DB::table('patients')
                          ...
                          ->where('appointments.agenda_id', $id)
                          ->groupBy('patients.id')
                          ->get(); 

            return View::make('clinic.doctor.Agendas.Patient.patients')->with('patients', $patients);    
    }

    public function getAppointmentsPending($agenda, $patient)
    {
        if(Payment::VeryPayment()==false)
           return View::make('clinic.payment.renews-payment');

        if(Agenda::getVeryAgenda($agenda)==false)
           return View::make('clinic.doctor.404');

        $Appointment = Appointment::where('state', 'pending')->where('patient_id', $patient)->where('agenda_id', $agenda)->get();
        return View::make('clinic.doctor.Agendas.Patient.pending')->with('Appointment', $Appointment);
    }

    public function getAppointmentsHistory($agenda, $patient)
    {
        if(Payment::VeryPayment()==false)
           return View::make('clinic.payment.renews-payment');
        ...
        $Appointment = Appointment::where('patient_id', $patient)->where('agenda_id', $agenda)->get();
        
        return View::make('clinic.doctor.Agendas.Patient.history')->with('Appointment', $Appointment);
    }
